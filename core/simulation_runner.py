...
from utils.utils import find_case_insensitive_path, comment_reference_line, duplicate_print_line
...
class SimulationRunner:
    ...
    def run_simulation(self, spice_file: str, canvas, fig):
        ...
        try:
            ...
            reference_result_file = os.path.join(REFERENCE_MODEL_CODE_PATH, "reference_data.txt")

            if not os.path.exists(reference_result_file):
                ...
                comment_reference_line(spice_file=spice_file)
            ...
            if not os.path.exists(user_result_file):
                raise RuntimeError(f"Не удалось создать файл {user_result_file}")

            print(f"Пользовательские данные успешно созданы: {user_result_file}")

            print(f"Чтение эталонных данных из {reference_result_file}")
            self.manager.run(
                fig=fig,
                canvas=canvas,
