...
class FileManager:
    ...
    def apply_changes_to_file(
        current_parameters: Dict[str, Optional[str]], target_file: Union[str]
    ):
        ...
        parameter_lines = {
            param_name: None for param_name in current_parameters
        ...
        for param_name, line_index in parameter_lines.items():
            if line_index is not None:
                ...
                if match:
                    ...
                    updated_line = (
                        f"{prefix}{current_parameters[param_name]}"
                        + line[match.end():]
                    )
