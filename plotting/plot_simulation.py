...
class DataLoader(Protocol):
    def load_data(self, filename: Union[str]) -> Union[pd.DataFrame]:
        pass
    ...
    def load_data(self, filename: Union[str]) -> Union[pd.DataFrame]:
        with open(filename, "r") as f:
            ...
            for i, line in enumerate(lines):
                if ("Index" in line and "v-sweep" in line) or (
                    "Index" in line and "time" in line
                ):  # FIXME переделать условие
                    header_line_index = i
                    break
...
class Plotter(DataPlotter):
    def plot(
        ...
        for column in column_names[2:]:
            ...
            ax.plot(
                x_segmented, y_segmented, label=label, color=color, linestyle=linestyle
            )
