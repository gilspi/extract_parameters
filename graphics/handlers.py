...
class SimulatorHandlers:
    def __init__(
        self, params_box, file_button, fig, ax, canvas_plot, progress_bar, parent_window
    ):
        ...
        self.user_result_file = os.path.join(
            SIMULATION_RAW_DATA_PATH, "simulation_data.txt"
        )
    ...
    def set_configuration(self, model_name):
        ...
        if not config:
            self.__show_message_dialog(
                ("Ошибка"), ("Неверная конфигурация."), Gtk.MessageType.ERROR
            )
            return
        ...
        self.simulation_runner = SimulationRunner(
            self.parsing_file, self.simulation_manager, self.user_result_file
        )
    ...
    def update_parameters(self, parsing_file):
        ...
        try:
            ignore_params_loader = FileIgnoreParamsLoader(
                ignore_file=IGNORE_PARAMS_FILE
            )
            parser = ParameterParser(
                file_path=parsing_file, ignore_params_loader=ignore_params_loader
            )
            ...
            for param in parameters:
                self.__add_parameter_row(
                    self.params_box, param["name"], str(param["default_value"])
                )

            self.params_box.show_all()
            self.simulation_runner = SimulationRunner(
                parsing_file, self.simulation_manager, self.user_result_file
            )
        except Exception as e:
            self.__show_message_dialog(
                ("Ошибка"), ("Ошибка при загрузке параметров."), Gtk.MessageType.ERROR
            )
            return
    ...
    def choose_model(self, widget):
        ...
        if not self.simulation_runner:
            self.__show_message_dialog(
                ("Ошибка"), ("Сначала выберите файл параметров."), Gtk.MessageType.ERROR
            )
            return
        initial_dir = (
            os.path.dirname(self.parsing_file)
            if self.parsing_file
            else SPICE_EXAMPLES_PATH
        )
        dialog = Gtk.FileChooserDialog(
    ...
    def choose_spice_file(self, widget):
        ...
        try:
            if not self.parsing_file:
                self.__show_message_dialog(
                    ("Ошибка"),
                    ("Сначала выберите файл параметров."),
                    Gtk.MessageType.ERROR,
                )
                return
            parent_dir_name = os.path.basename(
                os.path.dirname(os.path.dirname(self.parsing_file))
            )
            initial_dir = os.path.join(SPICE_EXAMPLES_PATH, parent_dir_name)
            dialog = Gtk.FileChooserDialog(
                title=_("Выберите SPICE-файл"),
                parent=self.parent_window,
                action=Gtk.FileChooserAction.OPEN,
            )
            dialog.set_current_folder(initial_dir)
            ...
            self.__show_message_dialog(
                ("Ошибка"),
                ("Ошибка при загрузке параметров: ") + str(e),
                Gtk.MessageType.ERROR,
            )
            return
    ...
    def apply_changes(self, widget):
        ...
        if not self.simulation_runner:
            self.__show_message_dialog(
                _("Ошибка"), ("Модель не выбрана."), Gtk.MessageType.ERROR
            )
            return
        current_parameters = {}
        ...
        try:
            self.file_manager.apply_changes_to_file(
                current_parameters, self.parsing_file
            )
            self.__show_message_dialog(
                ("Уведомление"),
            ...
            self.__show_message_dialog(
                ("Ошибка"), ("Не удалось применить изменения."), Gtk.MessageType.ERROR
            )
            return
    ...
    def start_simulation(self, button):
        ...
        if not self.spice_file:
            self.__show_message_dialog(
                _("Ошибка"), ("Сначала выберите SPICE-файл."), Gtk.MessageType.ERROR
            )
            return
        ...
        def simulate():
            try:
                for progress in self.simulation_runner.run_simulation(
                    spice_file=self.spice_file, canvas=self.canvas_plot, fig=self.fig
                ):
    ...
    def on_save_csv(self, widget):
        if (
            ...
            dialog.format_secondary_text(
                _("Симуляция не была выполнена, нет данных для сохранения.")
            )
            dialog.run()
            dialog.destroy()
            return
        try:
            ...
            parser = ParameterParser(
                file_path=self.parsing_file, ignore_params_loader=ignore_params_loader
            )
            parser.save_simulation_results(
                input_file=self.user_result_file, directory=OUTPUT_DATA_PATH
            )
        except Exception as e:
    ...
    def on_press(self, widget, event):
        ...
        if self.original_xlim is None or self.original_ylim is None:
            self.original_xlim, self.original_ylim = (
                self.ax.get_xlim(),
                self.ax.get_ylim(),
            )
        ...
        if event.button == Gdk.BUTTON_PRIMARY:
            ...
            self.start_point = (
                ...
                y_lim[0]
                + ((allocation.height - event.y) / allocation.height)
                * (y_lim[1] - y_lim[0]),
            )
    ...
    def on_scroll(self, widget, event):
        ...
        y_center = y_lim[0] + ((allocation.height - event.y) / allocation.height) * (
            y_lim[1] - y_lim[0]
        )
